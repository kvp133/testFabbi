...
The codebase was audited end-to-end. This overview lists only the
**🔴 Critical** and **🟠 High** findings — 27 issues in total. Quality,
UX, performance, and convention items (🟡 Medium / 🟢 Low) were noted
during the audit but are out of scope for this report.

**Eight fixes were shipped** on dedicated branches, closing every 🔴
Critical issue. The selection prioritises **correctness, security, and
data isolation** over cosmetic cleanup, as the assessment brief
instructs.
...
### Severity totals (Critical + High only)
...
| 7   | `bugfix/users-email-uniqueness`       | DB            | D1               | [`07-users-email-uniqueness.md`](./07-users-email-uniqueness.md) |
...
- **DB (D1)** is shipped together with a case-insensitive email
  normalisation pass, because a case-sensitive UNIQUE constraint would
  still allow `demo@x` and `DEMO@x` to coexist. Both the constraint and
  the normalisation have to land in the same migration.

### Why other High-severity items were not shipped

| Skipped         | Why                                                                                                                       |
| --------------- | ------------------------------------------------------------------------------------------------------------------------- |
| S4 (login email enumeration) | One-line response unification; trivial to add in a follow-up.                                                |
...
| B1, B2          | Backend partial-update logic — wrong but does not corrupt or leak data. Worth a follow-up.                                |
| D2, D3, D4, D5  | Performance / schema hygiene; significant but lower priority than data isolation and authentication for an MVP review.    |
| F5              | Frontend refresh-token flow — UX regression rather than a security issue, and the access token life is short.             |
| I1, I2, I3, I4  | Test reliability and infra hygiene; should be addressed but not blocking the security fixes.                              |
